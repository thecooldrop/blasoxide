...
fn dsyr_driver(upper: bool, n: usize, incx: usize, lda: usize) {
    ...
    unsafe {
        dsyr(upper, n, 7., x.as_ptr(), incx, a.as_mut_ptr(), lda);
    }
...
fn test_dsyr() {
    ...
    for &n in SIZES.iter() {
        for &(incx, _incy) in STRIDES.iter() {
            dsyr_driver(false, n, incx, lda);
            dsyr_driver(true, n, incx, lda);
        }
    }
}
